...
| `rate_limit.enabled` | `false` | Rate limiting (slowapi) |
| `rate_limit.default` | `"60/minute"` | Domyślny limit per IP |
...
- Endpoint `/api/v1/monitor/health` nie wymaga autentykacji
...
| 429 | Rate limit exceeded (gdy włączony) |
...
- **Rate limiting** (slowapi) — opcjonalne, domyślnie wyłączone
